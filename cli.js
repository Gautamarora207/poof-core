...
async function printCELOBalance({ address, name }) {
  const goldToken = await kit.contracts.getGoldToken();
  const balance = await goldToken.balanceOf(address);
  console.log(`${name} CELO balance is`, balance.toString());
}

/** Display ERC20 account balance */
async function printERC20Balance({ address, name, tokenAddress }, currency) {
  if (currency === "celo") {
  ...
  console.log(
    ...
    await erc20.methods.balanceOf(address).call()
  );
}
...
async function deposit({ currency, amount }) {
  ...
  await printERC20Balance(
    { address: tornado._address, name: "Tornado" },
    currency
  );
  await printERC20Balance(
    { address: senderAccount, name: "Sender account" },
    currency
  );
  ...
  if (toBN(allowance).lt(toBN(tokenAmount))) {
    ...
    const approveTx = await erc20.methods.approve(
      tornado._address,
      tokenAmount
    );
    const approveTxRes = await kit.sendTransactionObject(approveTx, {
      from: senderAccount,
    });
    console.log("Approval txn:", await approveTxRes.getHash());
  }

  console.log("Submitting deposit transaction");
  const depositTx = await tornado.methods.deposit(
    toHex(deposit.commitment),
    []
  );
  const depositTxRes = await kit.sendTransactionObject(depositTx, {
    from: senderAccount,
    gas: 2e6,
  });
...
async function withdraw({
  ...
  if (relayerURL) {
    ...
    const { rewardAccount, netId, gasPrices, celoPrices, tornadoServiceFee } =
      relayerStatus.data;
    assert(
...
function calculateFee({
  ...
  const decimalsPoint =
    Math.floor(tornadoServiceFee) === Number(tornadoServiceFee)
      ? 0
      : tornadoServiceFee.toString().split(".")[1].length;
  const roundDecimal = 10 ** decimalsPoint;
  const total = toBN(fromDecimals({ amount, decimals }));
  const feePercent = total
    .mul(toBN(tornadoServiceFee * roundDecimal))
...
async function init({ rpc, noteNetId, currency = "dai", amount = "100" }) {
  ...
  if (inBrowser) {
    ...
    contractJson = await (
      await fetch("client/contracts/ERC20Tornado.json")
    ).json();
  ...
  isLocalRPC = netId > 42 && netId !== 44787;
  ...
  tornado = new kit.web3.eth.Contract(contractJson.abi, tornadoAddress);
  erc20 = new kit.web3.eth.Contract(erc20ContractJson.abi, tokenAddress);
}

async function main() {
  if (inBrowser) {
    const instance = { currency: "celo", amount: "0.1" };
    await init(instance);
